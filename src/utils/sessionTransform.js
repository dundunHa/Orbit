...
function transformSession(session, level = 0) {
  const duration = session.started_at && session.last_event_at
    ? Math.floor((new Date(session.last_event_at) - new Date(session.started_at)) / 1000)
    : 0;

  return {
    id: session.id?.slice(-4) || session.id || 'unknown',
    status: STATUS_MAP[session.status?.type] || 'pending',
    description: session.title || session.status?.description || 'No description',
...
export function buildSessionTree(sessions, activeSessionId) {
  ...
  const rootSessions = [];
  const mainSession = activeSessions.find(s => s.id === activeSessionId) || activeSessions[0];
